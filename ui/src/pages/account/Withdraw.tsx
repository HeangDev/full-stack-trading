            ...
            <div className="auth__form">
                <form onSubmit={handleSubmit(handleWithdarw)} autoComplete="off">
                    <div className="auth__form__container">
                        <TextField type="text" fullWidth disabled label="Balance"
                            value="1000"
                            {...register("balance")}
                        />
                        <TextField type="text" fullWidth label="Withdraw Amount"
                            error={!!errors.amount}
                            helperText={errors.amount?.message}
                            {...register("amount")}
                        />
                        <TextField type="text" fullWidth label="Withdraw Code"
                            error={!!errors.code}
                            helperText={errors.code?.message}
                            {...register("code")}
                        />
                        <TextField type="text" fullWidth disabled label="Withdraw Fee"
                            {...register("fee")}
                        />
                        <TextField type="text" fullWidth multiline label="Withdraw Reason"
                            row={3}
                            {...register("reason")}
                        />
                    </div>
                    <div className="buttonPanel__fixed__container">
                        <div className="buttonPanel__fixed__box">
                            <Button type="submit" color="primary" onClick={() => setOpenPopup(true)}>Withdraw</Button>
                        </div>
                    </div>
                </form>
            </div>
